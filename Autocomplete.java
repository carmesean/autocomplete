...
/**
 * @main collab to TrieAutocomplete Carmen Stowe
 **/
...
public class Autocomplete {
   ...
   public static class TrieAutocomplete implements Autocompletor {
      ...
      private void add(String word, double weight) {
         ...
         //Creates any necessary intermediary nodes, does not add if the node already exists
         for (int i = 0; i < word.length(); i++) {
            if (!curr.children.containsKey(word.charAt(i))) {
               Node tempNode = new Node(word.charAt(i), curr, weight);
               curr.children.put(word.charAt(i), tempNode);
            }
            curr = curr.children.get(word.charAt(i));
            
            //Updates the subtreeMaxWeight of all nodes in the path from root to the node
            //*representing word.
      ...
      public Iterable<String> topMatches(String prefix, int k) {
         ...
         //only consider nodes that have at least the prefix
         Node curr = myRoot;
         LinkedList<String> ret = new LinkedList<String>();
         char[] myArray = prefix.toCharArray();
         for (int i = 0; i < myArray.length; i++) {
            Node validNode = curr.getChild(myArray[i]);
            if (validNode == null) {
               return ret;
            }
            curr = validNode;
         } 
         if (curr == null || curr.children.size() <= 0) {
         ...
         ArrayList<Node> termsList = new ArrayList<Node>(k); //list for terms
         pq.add(curr); //add root node to pq
         while(!pq.isEmpty()){ //while there are nodes left in pq
            curr = pq.remove(); //pop the root
            for(Node childNode : curr.children.values()) { //for each child node in curr
               pq.add(childNode); //add the child nodes back to pq
            }
            if (curr.isWord == true) { //if curr is a word
               termsList.add(curr); //add term object with curr values to termList
            }
         }  
         //int numResults = Math.min(k, termsList.size());
         if (k > termsList.size()){
            k = termsList.size();
         }
         for(int i = 0; i < k; i++){
            ret.add(termsList.get(i).getWord()); //add elements to ierable linkedList
         }  
         return ret; //return elements
      }
      ...
      public String topMatch(String prefix) {
         ...
         PriorityQueue<Node> pq = new PriorityQueue<Node>(1, new Node.ReverseSubtreeMaxWeightComparator());
         ArrayList<Node> termsList = new ArrayList<Node>(1); //list for terms
         pq.add(curr); //add root node to pq
         while(!pq.isEmpty()){ //while there are nodes left in pq
            curr = pq.remove(); //pop the root
            for(Node childNode : curr.children.values()) { //for each child node in curr
         ...
         int j = 0;
         return termsList.get(j).getWord();          
      }
                 /**
         * Return the weight of a given term. If term is not in the dictionary,
         * return 0.0
